    ...
    public class GoSystems
    {
        ...
        public void MovimentBehaviour(float speed, float fastRunSpeed, GoInput input, MoveInput moveInput)
        {
            ...
            if (!IsLockMove)
            {
                ...
                {
                    Grigidbody.linearVelocity = new Vector3(moveDir.x * savespeed, Grigidbody.linearVelocity.y, moveDir.z * savespeed);
                }
        ...
        public void CrouchPosAction(GoInput crouchActive, float speed)
        {
            ...
            if (IsLockMove || IsLockCrouch) return;
            ...
            var wallTargetCheck = ChackUp(target);
            var capsule = GoSystemsController.Charcter.GetComponent<CapsuleCollider>();
            animatorControler.SetBool(crouch, IsCrouch);

            if (getDown && !RagdallActivate && !couchSpam)
            {
                if (Crouch == 0 && !LockCrouch) // Enter crouch
                {
                ...
                else if (Crouch == 1 && LockCrouch && !wallTargetCheck) // Exit crouch
                {
                    Crouch = 3;
                    couchSpam = true;
                    IsCrouch = false;
            ...
            // If not grounded while crouching, reset crouch state
            ...
            if (getinput && !IsLockJump)
            {
                ...
                if (IsGraunded(layer))
                {
                    ...
                    if (IsJumping)
                    {
                        ...
                        Grigidbody.AddForce(Grigidbody.linearVelocity.x, Grigidbody.linearVelocity.y + initialJumpVelocity * force, Grigidbody.linearVelocity.z);
                        IsGround = false;
        ...
        private float Velocity(float x, float z)
        {
            ...
            if (x > 0 && z > 0)
            {
                return Mathf.Min(velocity, 1);
            }
            else if (x < 0 && z < 0)
            {
                return Mathf.Min(-velocity, 1) / 2;
            }
